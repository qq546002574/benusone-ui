...
export default {
  ...
  watch: {
    value(val) {
      ...
      console.log("val", val);
      if (val == 1) {
        this.currentMoveClientX =
          Math.round(this.clientWidth * 0.16 * 100) / 100;
  ...
  methods: {
    ...
    touchMove(event) {
      ...
      if (
        (that.value == 1 && that.moveClientX > 0) ||
        (that.value == that.alength && that.moveClientX < 0) ||
        that.isTransion
      ) {
        that.moveClientX = 0;
        return;
      ...
      if (Math.abs(that.moveClientX) < that.clientWidth) {
        ...
        that.$refs.ul.style.transform = `translateX(${
          that.currentMoveClientX + that.moveClientX
        }px)`;
      }
    },
    touchEnd() {
